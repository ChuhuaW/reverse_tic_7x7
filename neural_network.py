...
def update_nn_training_set(new_games, training_set):
    ...
    for game in new_games:
        ...
        for i in range(len(game[0])-1):
            ...
            y = np.concatenate([one_hot_winner, move], axis=0).reshape(-1, 52)
            x = initial.reshape(-1, 49)
...
class model_two_hidden():
    def __init__(self, size_x=49, size_y=52):
        self.size_x = size_x
        self.size_y = size_y
